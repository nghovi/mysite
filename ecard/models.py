...
class LinkSerializer(serializers.ModelSerializer):
    # id =  serializers.IntegerField()# Must have to do this or else id will be read only and cannot appear on validated_data
    class Meta:
        model = Link
...
class Book(models.Model):
    ...
    comment = models.CharField(max_length=500, blank=True, default='') #default = '' doesn't mean field can be blank or '' in form
    link = models.ForeignKey(Link, default=None, blank=True, null=True)#only set null=True for non-string fields
    created = models.DateTimeField(auto_now_add=True)
    modified = models.DateTimeField(auto_now=True)
    ...
    def update(self, instance, validated_data):
        ...
        for phrase in phrase_data:
            if phrase.get('id') in ids:
                PhraseSerializer().update(instance=instance.get_phrase(phrase.get('id')), validated_data=phrase)
            else:
                Phrase.objects.create(word=instance, content=phrase.get('content'))
...
class BookSimpleSerializer(serializers.ModelSerializer):
    link = LinkSerializer(many=False, read_only=False) #If readonly=True, link wil not appear in validated_data of create, though it'll in form
    class Meta:
        model = Book
    ...
    def create(self, validated_data):
        ...
        link =  Link(url=validated_data.get('link').get('url'), description="Link of Book " + validated_data.get('name'))
        link.save()
        new_book = Book(link=link, name=validated_data.get('name'), author=validated_data.get('author'), comment=validated_data.get('comment'), iconurl=validated_data.get('iconurl'))
        new_book.save()
...
class BookSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        """
        with nested serializer i will update data for each child, not updated whole in their parent
        ...
        if instance.link is None:
            ...
            LinkSerializer().update(instance=instance.link, validated_data=link_data)
